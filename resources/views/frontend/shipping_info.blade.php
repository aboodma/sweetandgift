            ...
            <div class="col-xxl-8 col-xl-10 mx-auto">
                <form class="form-default" data-toggle="validator" action="{{ route('checkout.store_shipping_infostore') }}" role="form" method="POST">
                    @csrf
                        ...
                        @elseif(Session::has('trend_id'))
                        <div class="shadow-sm bg-white p-4 rounded mb-4">
                            <div class="form-group">
                                <label class="control-label">{{ translate('Name')}}</label>
                                <input type="text" class="form-control" name="name" value="{{$data[0]->user->name}}" placeholder="{{ translate('Name')}}" required>
                            </div>

                            <div class="form-group">
                                <label class="control-label">{{ translate('Email')}}</label>
                                <input type="text" class="form-control" name="email" value="{{$data[0]->user->email}}" placeholder="{{ translate('Email')}}" required>
                            </div>

                            <div class="form-group">
                                <label class="control-label">{{ translate('Address')}}</label>
                                <input type="text" class="form-control" name="address"value="{{$data[0]->hotel->country}} - {{$data[0]->hotel->city}} - {{$data[0]->hotel->address}} - Hotel Name : {{$data[0]->hotel->name}} - Room Number: {{$data[0]->room->room_number}}" placeholder="{{ translate('Address')}}" required>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group has-feedback">
                                        ...
                                        <input type="text" class="form-control" placeholder="{{ translate('Postal code')}}" value="34000" name="postal_code" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group has-feedback">
                                        <label class="control-label">{{ translate('Phone')}}</label>
                                        <input type="number" lang="en" min="0" class="form-control" value="{{$data[0]->phone}}" placeholder="{{ translate('Phone')}}" name="phone" required>
                                    </div>
                                </div>
                            </div>
                            <input type="hidden" name="checkout_type" value="trend_user">
                        </div>
